...
public class ErrorDialogManager {
    ...
    public static class SupportManagerFragment extends Fragment {
        ...
        public void onEventMainThread(event.util.ThrowableFailureEvent event) {
...
//            }
        ...
        public static void attachTo(Activity activity, Object executionScope, boolean finishAfterDialog,
            ...
            if (fragment == null) {
                ...
                fm.beginTransaction().add(fragment, TAG_ERROR_DIALOG_MANAGER).commitAllowingStateLoss();
                fm.executePendingTransactions();
            }
...
//            }
    ...
    public static event.util.ErrorDialogFragmentFactory<?> factory;
    ...
    public static final String KEY_ICON_ID = "de.greenrobot.eventbus.errordiaLogUtil.icon_id";
    ...
    protected static void checkLogException(event.util.ThrowableFailureEvent event) {
        if (factory.config.logExceptions) {
            String tag = factory.config.tagForLoggingExceptions;
            if (tag == null) {
                tag = EventBus.TAG;
            }
            LogUtil.i(tag, "Error dialog manager received exception");
        }
    }

    private static boolean isInExecutionScope(Object executionScope, event.util.ThrowableFailureEvent event) {
        if (executionScope != null && event instanceof event.util.HasExecutionScope) {
