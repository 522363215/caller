package blocker.call.wallpaper.screen.caller.ringtones.callercolor.async;
    ...
    private static ThreadPoolExecutorWrapper getThreadPoolExecutorWrapper() {
        if (sThreadPoolExecutorWrapper == null) {
            synchronized (Async.class) {
                if (sThreadPoolExecutorWrapper == null) {
                    sThreadPoolExecutorWrapper = new ThreadPoolExecutorWrapper(4, 8, 6);
//                    if (BuildConfig.DEBUG) LogUtil.d(LogFilterDef.APP_INIT, LogHelper.getFileLineMethod(1));
                }
            }
        }
