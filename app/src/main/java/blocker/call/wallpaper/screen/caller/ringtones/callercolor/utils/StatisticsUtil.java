...
//            object.put("imei", DBAssistant.encrypt(DeviceUtil.getIMEI(context)));
     ...
     * 上传notifycation点击统计
     */
    public static void statisticsNotificationClick(final String flag, final Context context) {
        if (!TextUtils.isEmpty(flag)) {
            HttpUtil.postNofityData(context, HttpUtil.ACTION_TYPE_CLICK, flag, new AjaxCallback<JSONObject>() {
                @Override
                public void callback(String url, JSONObject object, AjaxStatus status) {
                    if (object == null) {
                        SharedPreferences wifiAcceleration = context.getSharedPreferences("wifiAcceleration", Context.MODE_PRIVATE);
                        SharedPreferences.Editor edit = wifiAcceleration.edit();

                        int i = wifiAcceleration.getInt(flag, 0);
     ...
     * @param flag
     */
    public static void postShowNotifyCation(final Context context, final String flag) {
        new Thread() {
            @Override
            public void run() {
                HttpUtil.postNofityData(context, HttpUtil.ACTION_TYPE_SHOW, flag, new AjaxCallback<JSONObject>() {
                    @Override
                    public void callback(String url, JSONObject object, AjaxStatus status) {
                        if (object == null) {
                            SharedPreferences wifiAcceleration = context.getSharedPreferences("wifiAcceleration", Context.MODE_PRIVATE);
